...
SpotLight::SpotLight(GLfloat red, GLfloat green, GLfloat blue, GLfloat aIntensity, GLfloat dIntensity, 
	...
	PointLight(red, green, blue,aIntensity, dIntensity, xPos, yPos, zPos, cons, lin, exp) {
	direction = glm::normalize(glm::vec3(xDirection, yDirection, zDirection));
	edge = edg;
	procEdge = cosf(glm::radians(edge));
}

void SpotLight::useLight(GLuint ambientIntensityLocation, GLuint ambientColorLocation, GLuint diffuseIntensityLocation, GLuint positionLocation, GLuint directionLocation,
